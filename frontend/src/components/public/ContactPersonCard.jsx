export default function ContactPersonCard({
  ...
  return (
    <div className={`rounded-[1.75rem] border p-5 ${accentClasses}`}>
      <div className="text-xs uppercase tracking-[0.28em] text-sliit-muted">{title}</div>
      <div className="mt-3 text-xl font-semibold text-white">{person.name}</div>
      <div className="mt-3 space-y-2 text-sm text-sliit-muted">
        <p><span className="text-white">Email:</span> {person.email}</p>
        {person.phone ? <p><span className="text-white">Phone:</span> {person.phone}</p> : null}
      </div>
